...
input1 = read_excel("C:/Users/majidmm/Documents/UPD/Email disposition/Station_email_disposition.xlsx", sheet = 'UPD Portal - North America (NA)')
input2 = read_excel("C:/Users/majidmm/Documents/UPD/Email disposition/Station_email_disposition.xlsx", sheet = 'Response level')
...
station<-unique(raw$`Station Short Code`)
...
{
  raw$`Station email`[k]<-gsub(";", ",", raw$`Station email`[k])
  raw$emaillength[k]<-length(unlist(strsplit(raw$`Station email`[k], ",")))
...
{
  temp<-raw%>%filter(raw$`Station Short Code`== station[m])
  for(x in 1:length(temp$`Station Short Code`))
  {
    temp$`Station email`[1] = paste(temp$`Station email`[1],",",temp$`Station email`[x])
    temp$`Station email`[1] = gsub(" ", "",temp$`Station email`[1])
  }
  z1 = gsub(" ", "",temp$`Station email`[1])
  z2 = unlist(strsplit(z1, ","))
  z3 = lapply(z2, tolower)
  z4 = unique(z3)
  z5 = do.call(paste, c(as.list(z4), sep = ","))
  temp$`Station email`[1] = z5
  ...
  list = unlist(strsplit(temp$`Station email`[1], ","))
  ...
  #outMail[["subject"]] = paste("UPD Report " , "Week-",temp$`Submission Week`[1], station[m])
  body<-"
...
If you would like an additional review of the requests pertaining to delays at the station (Under the roof), please reach out to the station management and email amzl-dspinvoice@amazon.com.                                      
  ...
  subject <- paste("UPD Report -",station[m],"-","Week",temp$`Submission Week`[1])
  ...
  tryCatch( { send.mail(from=from,
				...
				bcc=bcc,
                        ...
                        send = TRUE,
                        attach.files = c(tf),
                        file.names = c(paste("UPD Report -",station[m],"-","Week",temp$`Submission Week`[1],".xlsx")))}, error = function(e) {print(station[m])})
  #outMail$Send()
}